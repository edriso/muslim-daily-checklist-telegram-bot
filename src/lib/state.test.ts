 ...
 * rule — so it must behave defensively: a missing file is the routine
 * first-boot case, a corrupt file must not poison memory, and a write
 * failure must not throw out of the cron tick.
 *
...
beforeEach(async () => {
  tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'mdcb-state-'));
...
describe('initState', () => {
  it('starts empty when the file does not exist', async () => {
    ...
    expect(getLastMessageId('morning_azkar')).toBeUndefined();
  });
...
describe('setLastMessageId', () => {
  it('round-trips a value through memory', async () => {
    await initState(tmpFile);
    await setLastMessageId('morning_azkar', 42);
    expect(getLastMessageId('morning_azkar')).toBe(42);
  });

  it('persists changes to disk so a fresh init recovers them', async () => {
    await initState(tmpFile);
    await setLastMessageId('morning_azkar', 7);
    await setLastMessageId('evening_azkar', 8);

    _resetForTests();
    await initState(tmpFile);

    expect(getLastMessageId('morning_azkar')).toBe(7);
    expect(getLastMessageId('evening_azkar')).toBe(8);
  });

  it('overwriting a value keeps only the newest', async () => {
    await initState(tmpFile);
    await setLastMessageId('friday_sunnah', 100);
    await setLastMessageId('friday_sunnah', 200);

    _resetForTests();
    await initState(tmpFile);

    expect(getLastMessageId('friday_sunnah')).toBe(200);
  });
